    ...
    def learning(self, gamma, alpha, max_episode_num):
        ...
        while num_episode <= max_episode_num:
            ...
            while not is_done:
                ...
                self._assert_state_in_Q(s1, randomized = True)        #1: randomized = True ; 2:randomized=False
                # 在下行代码中添加参数use_epsilon = False即变成Q学习算法
                a1 = self.performPolicy(s1, num_episode, use_epsilon=False)
                a2 = self.performPolicy(s1, num_episode, use_epsilon=True)
                ...
                #just a test, to understand how get_Q() works
                #print("old_q={0:.3f}, q_prime={1:.3f}, td_target={2:.3f}, new_q={3:.3f}".format(old_q,q_prime,td_target,new_q))
                #print("Q[{0}][{1}] = {2:.3f}".format(int(s0),int(a0),self.Q[s0][a0]))
            ...
            num_episode += 1
        return self.Q
    def running(self, max_episode_num):
        total_time, time_in_episode, num_episode = 0, 0, 0
        while num_episode <= max_episode_num:
            self.state = self.env.reset()
            s0 = self._get_state_name(self.state)
            self.env.render()
            a0 = self.newPolicy(s0, num_episode)
            ...
            while not is_done:
                ...
                self._assert_state_in_Q(s1, randomized = True)        #1: randomized = True ; 2:randomized=False
                # 在下行代码中添加参数use_epsilon = False即变成Q学习算法
                a1 = self.newPolicy(s1, num_episode)
            ...
            total_time += time_in_episode
            num_episode += 1
    def newPolicy(self,s,episode_num):
        Q_s = self.Q[s]
        str_act = max(Q_s, key = Q_s.get)
        action = int(str_act)
        return action
...
def main():
    ...
    agent.learning(gamma = 0.9,
                   alpha = 0.1,
                   max_episode_num = 500)
    agent.running(max_episode_num=100)
