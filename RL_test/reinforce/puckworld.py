...
class PuckWorldEnv(gym.Env):
    '''children of gym.Env (see class Env on website for details)
    #to override
        _step()
        _reset()
        _render()
    ...
    def __init__(self):
        '''set action_space, obervation_space, reward
        ...
        self.update_time = 500      # time for target randomize its position
        # 作为观察空间每一个特征值的下限
    ...
    def _seed(self, seed=None):
        '''产生一个随机化时需要的seed和一个np_random对象(支持后续的随机化生成操作
    ...
    def _step(self, action):
        '''take action and then renew parameters of PuckWorldEnv
    ...
    def _random_pos(self):
        '''get a random number 0~1 with method: self.np_random.uniform(low=0, high=1)

        #return
            a scalar
        '''
        return self.np_random.uniform(low = 0, high = self.l_unit)  #0~1

    def _compute_dis(self, dx, dy):
        '''compute current distance

        #args
            dx,dy: current position to goal position<float>
    ...
    def _render(self, mode='human', close=False):   #不太懂
        '''renew GUI of current state
...
if __name__ =="__main__":
    ...
    print("hello")
    env.reset()
    nfs = env.observation_space.shape[0]   #每个状态（state或obs）的特征数
