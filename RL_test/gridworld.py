...
class Grid(object):
    def __init__(self, x:int = None, 
        ...
        self._update_name()
        #print(self.__str__())      #for test
    def _update_name(self):
        self.name = "X{0}-Y{1}".format(self.x, self.y)

    def __str__(self):
        return "name:{5}, x:{0}, y:{1}, type:{2}, reward{3}, value{4}".format(self.x,
                                                                   self.y,
                                                                   self.type,
                                                                   self.reward,
                                                                   self.value,
...
class GridMatrix(object):
    ...
    def reset(self):
        self.grids = []
        for y in range(self.n_height):  #此处有问题，应该是y in n_height
...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, n_width:int=10,
        ...
        #self.action_space = spaces.Discrete(4)  #only 0,1,2,3
        self.action_space = spaces.Discrete(8)  #Discrete(n) means 0,1,2...n-1
    ...
    def _step(self, action):
        ...
        elif action == 4: new_x,new_y = new_x-1,new_y-1         #left down
        elif action == 5: new_x,new_y = new_x-1,new_y+1        #原来此行与第四行一模一样 left up
        elif action == 6: new_x,new_y = new_x+1,new_y-1        #right down
        elif action == 7: new_x,new_y = new_x+1,new_y+1         #right up
        # boundary effect
        if new_x < 0: new_x = 0
    ...
    def _xy_to_state(self, x, y = None):
        if isinstance(x, int):
            ...
            return x + self.n_width * y         #源代码x，y反了
        elif isinstance(x, tuple):
            return x[0] + self.n_width * x[1]   #源代码x，y反了
        return -1        # 未知状态
